...
$dbname = $modx->db->config['dbase']; //nome del database
$dbprefix = $modx->db->config['table_prefix']; //prefisso per le tabelle
$charset = $modx->db->config['charset']; //codifica del database
$mod_table = $dbprefix."site_easyadvt"; //modulo tavolo
$theme = $modx->config['manager_theme']; //argomento di amministrazione
$ui_theme = (isset($ui_theme) && $ui_theme != '') ? $ui_theme : 'smoothness'; // Tema per jquery-ui, dal modulo di configurazione
...
////Установка модуля (создание таблицы в БД)
	...
	default:
		...
		if ($modx->db->getRecordCount($modx->db->query("show tables from ".$dbname." like '".$mod_table."'"))==0)
			...
			echo '<ul class="actionButtons"><li><a href="#" onclick="postForm(\'install\',null);return false;"><img src="media/style/'.$theme.'/images/icons/save.png" align="absmiddle" />set modulo</a></li></ul>';
		
		else {
		
			$but_add = '<ul class="actionButtons"><li><a href="#" onclick="addRow(0);return false;"><img src="media/style/'.$theme.'/images/icons/add.png" align="absmiddle" />Aggiungi</a></li></ul>';
			...
			<div class="dright">
				<ul class="actionButtons">
					<li><a href="#" onclick="if(confirm(\'Вы уверены?\n\rTutti i dati saranno eliminati definitivamente.\')){postForm(\'uninstall\',null)};return false;">
						<img src="media/style/'.$theme.'/images/icons/delete.png" align="absmiddle" />Cancella modulo</a>
					</li>
				</ul>
			</div>
			<div class="clear"></div>
