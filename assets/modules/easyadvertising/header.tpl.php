			...
			jq.post(
				...
				function(data) {
					if (data == 1)
						...
						uiMess('If you try to remove the error occurred.', 'Error');
				}
			);
		}
								...
								jq.post(
									...
									function(data) {
										if (data == 1)
											...
											uiMess('When you try to save the data error has occurred.', 'Error');
									}
								);
								jq(this).dialog("close");
							},
		...
		jq(document).ready(function(){
			...
			jq("table#flex").flexigrid({
				...
				colModel : [
						...
						{display: 'Priorità', name : 'pos', width : <?php echo $w[1]; ?>, sortable : true, align: 'left'},
						{display: 'Template URL', name : 'template', width : <?php echo $w[2]; ?>, sortable : true, align: 'left'},
						{display: 'Posizione', name : 'area', width : <?php echo $w[3]; ?>, sortable : true, align: 'left'},
						{display: 'Descrizione', name : 'description', width : <?php echo $w[4]; ?>, sortable : true, align: 'left'},
						{display: 'Inizio', name : 'pub_date', width : <?php echo $w[5]; ?>, sortable : true, align: 'center'},
						{display: 'Fine', name : 'unpub_date', width : <?php echo $w[6]; ?>, sortable : true, align: 'center'},
						{display: 'Più attivi', name : 'published', width : <?php echo $w[7]; ?>, sortable : true, align: 'center'},
						{display: 'Impression', name : 'count_view', width : <?php echo $w[8]; ?>, sortable : true, align: 'left'},
						{display: 'Limite', name : 'total_view', width : <?php echo $w[9]; ?>, sortable : true, align: 'left'},
						{display: 'Clicks', name : 'jump_count', width : <?php echo $w[10]; ?>, sortable : true, align: 'left'},
						{display: '', name : 'edit', width : 20, sortable : false, align: 'center'},
						{display: '', name : 'del', width : 20, sortable : false, align: 'center'}
						],
				searchitems : [
						{display: 'URL Pattern', name : 'template', isdefault: true},
						{display: 'Promotional zone', name : 'area'},
...
<br />
<div class="sectionHeader">Easy Advertising - Gestione della pubblicità sul sito</div>
